...
def GetCity01():
    ...
    sql="SELECT user_id,city_type FROM t_user_city_distr where city_type=1 limit 10000;"
...
def main():
    ...
    for i in datedicts.items():
        try:
            ...
            data_temp=data_test.merge(data_01,on='user_id')
            data_temp['prices'] = data_temp['prices'].astype('int64')
            if len(data_temp[data_temp['city_type']=='1'])>0:
                Sumprice=data_temp[data_temp['city_type']=='1']['prices'].sum()
                CountOrders=data_temp[data_temp['city_type']=='1']['city_type'].count()
                Usercount=pd.DataFrame(data_temp[data_temp['city_type'] == '1']['user_id']).drop_duplicates()['user_id'].count()
    ...
    result_fy.to_excel(r'C:\Users\Administrator\Desktop\city_price_01_new.xlsx')
