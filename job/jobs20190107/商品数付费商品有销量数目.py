...
def getdata(dt):
    ...
    sql_audio = 'select count(*) from t_audio where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type in (1,2) and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_image = 'select count(*) from t_image_text where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type in (1,2) and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_video = 'select count(*) from t_video where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type in (1,2) and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_ebook = 'select count(*) from t_ebook where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type in (1,2) and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_pay_product = 'select count(*) from t_pay_products where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and sell_type=1 and name not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_alive = 'select count(*) from t_alive where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type in (1,2) and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
###商品数
...
###付费商品数目
    sql_pay_audio = 'select count(*) from t_audio where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type =2 and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_pay_image = 'select count(*) from t_image_text where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type =2 and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_pay_video = 'select count(*) from t_video where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type =2 and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_pay_ebook = 'select count(*) from t_ebook where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type =2 and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_pay_pay_product = 'select count(*) from t_pay_products where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and sell_type =1 and price>0 and name not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
    sql_pay_alive = 'select count(*) from t_alive where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(str(
        dt)) + 'and payment_type =2 and title not in ("一分钟了解小鹅通（体验内容）","图文教程(体验内容，支付后可提现)","专栏教程","验证支付信息专用商品，购买后请删除","测试")'
...
###有销量商品
    sql_orders = 'select count(distinct product_id) from t_orders where DATE_FORMAT(created_at,"%Y-%m")<=' + '"{}"'.format(
...
def main():
    ...
    for i in ('2016','2017','2018'):
        for j in ('01','02','03','04','05','06','07','08','09','10','11','12'):
            dt=i+'-'+j
            result.append(getdata(dt))
            print('成功跑完：'+dt)
    data=pd.DataFrame(result,columns=['dt','products','payproducts','countproducts'])
    data.to_excel(r'D:\job_xiaoe\job_20190107\productsresult.xlsx')
